...
void LivoxPointsPlugin::OnNewLaserScans() {
    if (rayShape) {
        ...
        scan_point.header.frame_id = "livox"; // raySensor->Name();

        scan_point.channels.resize(1);
        scan_point.channels[0].name = "intensity";
        auto &scan_intensities = scan_point.channels[0].values;
        ...
        for (auto &pair : points_pair) {
            ...
            //   auto index = (verticalRayCount - verticle_index - 1) * rayCount + horizon_index;
                ...
                scan_intensities.push_back(static_cast<float>(intensity));
            //} else {
        ...
        sensor_msgs::convertPointCloudToPointCloud2(scan_point, scan_point2);

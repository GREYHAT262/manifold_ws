...
{
    ros::NodeHandle nh;
    config_yaml_path = "/manifold_ws/src/tracking/config/config.yaml";
...
{   
    // pure pursuit is disabled when robot is in collision recovery mode
    bool pure_pursuit_enabled;
    ros::param::param<bool>("/pure_pursuit_enabled", pure_pursuit_enabled, true);
    ...
    double cmd_omega_scale = 2;

    if (L > 0.1) {  // 避免除以零或很小的值
        double alpha = std::atan2(target_y, target_x);
        cmd_omega = cmd_omega_scale * 2.0 * cmd_v * std::sin(alpha) / L;
    } else {
        // 如果目标点太近，使用简单的方向控制

    ...
    : mapping_node_(mapping_node), tf_subscriber_node_(tf_subscriber_node)
{
    ...
    config_yaml_path = "/manifold_ws/src/navigation/config/config.yaml";
...
void AStarPlanningNode::TimerCallback(const ros::TimerEvent& event)
{
    ...
    // 计算起点和终点的栅格坐标 // Calculate start and goal grid coordinates
    ...
    // 尝试复用 last_path_：使用"放宽版"地图，只考虑车身本体，无安全冗余 // Test the last_path_ on relaxed_grid_map (no safety buffer)
    if (!last_path_.empty() && 
        ...
        // Starting from safety_hor, gradually decrease the safety buffer until a valid path is found
        while (current_safety >= 0.0) {
            // 创建临时地图用于A*搜索，从原始地图复制 // Start with origin_map
            DenseGridMap temp_map = mapping_node_->origin_map;
            
            // 使用新的boundedInflation接口直接在地图上执行膨胀 // Inflate the map based on current_safety
            temp_map.boundedInflation(std::max(mapping_node_->car_length/2.0, mapping_node_->car_width/2.0) + current_safety);
            
            // 确保起点和终点可通行 // Ensure the start and goal are not viewed as obstacles

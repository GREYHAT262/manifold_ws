    ...
    grid_map(0.1, 100)
{
    ...
    // Publishing goal for rviz
    visual_timer_ = nh.createTimer(ros::Duration(1.0), &MappingNode::timerCallback, this);
...
void MappingNode::timerCallback(const ros::TimerEvent& event) {
    if (goal_state_.have_goal_) {
        publishGoalMarker();
        /* ROS_INFO("Goal Marker Heartbeat: x=[%f], y=[%f]", 
                 goal_state_.current_goal_.pose.position.x, 
                 goal_state_.current_goal_.pose.position.y); */
    } else {
        ROS_INFO("Timer running, but no goal received yet (ready: %s)", ready ? "true" : "false");
    }
}
